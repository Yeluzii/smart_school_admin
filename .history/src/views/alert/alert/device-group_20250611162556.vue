<template>
  ...
  <el-card>
    <el-space>
      <el-space>
        <el-button icon="Plus" type="primary" @click="addBatchDeviceHandle()">添加设备</el-button>
      </el-space>
    </el-space>
...
const addBatchDeviceHandle = (key?: any) => {
  ...
  ElMessageBox.confirm('确定进行添加操作?', '提示', {
    ...
    .then(() => {
      console.log('alertId', props.alertId)
      console.log('data', data)
      const ids = Object.values(data)
      console.log('ids', ids)
      service.post('new/sys/alert/batchsave', { ids, alertId: props.alertId }).then(() => {
